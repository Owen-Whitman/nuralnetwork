...
class NN:
	...
	def sigmanuts(self, num):
		if(self.flattenfunction < 0):
			#step function with slope
			if(num >= (self.flattenfunction*-100)):
				anw = 1.0
			if(num < (self.flattenfunction*-100)):
				anw = 0
		elif(self.flattenfunction < 1 ):
			#linar function
			anw = num *(self.flattenfunction*100)
		elif(self.flattenfunction == 1):
			#step function
			if(num >= 0.5):
				anw = 1.0
				...
				anw = 0.0
		elif(self.flattenfunction == 2):
			#sigmoid function
			anw = 1 / (1+ math.exp(-num))
		elif(self.flattenfunction == 3):
			#ArcTan
			anw = math.atan(num)
		elif(self.flattenfunction == 4):
			#tanh
			anw = (math.exp(-2*num) - 1) / (math.exp(-2*num) + 1)
		elif(self.flattenfunction == 5):
			#ReLu
			anw = max(0,num)
		elif(self.flattenfunction == 6):
			#Leaky ReLu
			anw = max(0.01,num)
		elif(self.flattenfunction > 7 and self.flattenfunction < 8 or self.flattenfunction < -7 and self.flattenfunction > -8):
			#PReLU
			if(self.flattenfunction < -7 and self.flattenfunction > -8):
				if(num >= 0):
					anw = num
				else:
					anw = ((self.flattenfunction+7)*100) * num	
			else:
				if(num >= 0):
					anw = num
				else:
					anw = ((self.flattenfunction-7)*100) * num
		elif(self.flattenfunction > 8 and self.flattenfunction < 9 or self.flattenfunction < -8 and self.flattenfunction > -9):
			#ELU
			if(self.flattenfunction < -8 and self.flattenfunction > -9):
				if(num >= 0):
					anw = num
				else:
					anw = ((self.flattenfunction+8)*100) * (math.exp(num)-1)
			else:
				if(num >= 0):
					anw = num
				else:
					anw = ((self.flattenfunction-8)*100) * (math.exp(num)-1)			
		elif(self.flattenfunction == 9):
			#SoftPlus
			anw = math.log((1+math.exp(num)),math.exp(1))
		return anw
	...
	def calccost(self,perdictions):
		...
		print(cost)
		for layer in range(len(cost)-1,0,-1):
			print(layer,"g",len(self.numofnets)-layer-1)
			if(isinstance(self.numofnets[layer],list)):
			
					print(self.numofnets[len(self.numofnets)-layer])
					for layers in range(len(self.network[layer])):
						print(layers,"tsgdfg",len(self.numofnets)-layer)
						for connection in range(1,len(self.network[layer][layers])):
							...
							for neuornb in range(len(self.network[layer][layers][connection])-1):
								print(cost[len(self.numofnets)-layer],layer,'sgc')
